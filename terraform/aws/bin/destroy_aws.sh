...
destroy_lt() {
    for lt in ${REMAINING_LT} ; do
        echo "🚨 Attempting to destroy ${lt}..."
        aws ec2 delete-launch-template --launch-template-id ${lt} --region ${AWS_REGION}

        if [[ $? -eq 0 ]] ; then
            echo "Destroy LC ${lt} attempt was success."
        else
            echo "Destroy of LC ${lt} Failed. Error."
        fi
    done
}
